<template>
  ...
  <section class="box-header-home">
    <div class="box-search-header">
      <h1 class="home-title w-100 text-center">استخدام آژانس هواپیمایی و مسافرتی</h1>
...
export default {
  ...
  created() {
    if(Object.keys(this.$route.query).length != 0 )
    {
      ...
      this.$store.commit('Site/Advertising/SET_SEARCH_FORM' , this.form )
    }
    else {
      let nullForm =  {
        Job : '',
      ...
      this.$store.commit('Site/Advertising/SET_SEARCH_FORM' , nullForm )
    }
  },
  mounted() {
  ...
  methods: {
    ...
    SearchJob(){
      if (this.$v.$invalid) {
        ...
        this.loadingB = true;
        this.$store.commit('Site/Advertising/SET_SEARCH_FORM' , this.form )
        this.$store.dispatch('Site/Advertising/LoadAdvertisingList'  , this.searchParam ).then(response => {this.loadingB = false;   this.$emit('setfilter')});
        this.$router.replace(`/jobs?job1=${this.searchParam.Job}&job2=${this.searchParam.Job1}&province=${this.searchParam.province}`).catch(() => {});
      }
