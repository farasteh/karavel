<template>
    <div>
        ...
        <div style="padding-bottom: 10px;" class="row">
           ...
           <div class="col-6">
                <Button  @click="CreateSkill()" class="col-8 " style="float: left;" type="primary"><Icon style="font-size: 19px;font-weight: bold;" type="md-add" />ثبت مهارت رفتاری جدید</Button>
           </div>
        </div>
        ...
        <div  >
                ...
                <div slot="content" class="content" v-for="(skill , index) in behavioralSkills" :key="index">
                   <div class="row">
                        ...
                        <div class="col-6">
                            <Button  @click="EditSkill(skill.title , skill.id , index)" class="col-md-1 col-4 btn-admin" ><Icon style="font-size: 19px;font-weight: bold;display: flex;justify-content: center;" type="ios-create" /></Button>
                            <Button  @click="DeleteSkill(skill.title , skill.id , index)"   class="col-md-1 col-4 btn-admin" ><Icon style="font-size: 19px;font-weight: bold;display: flex;justify-content: center;" type="ios-trash" /></Button>
                        </div>
                    </div>
                </div>
...
export default {
        ...
        mounted() {
            this.$store.dispatch('LoadBehavioralSkills');
        },
        methods: {
            CreateSkill() {
                this.modal2 = true;
            ...
            newSkill() {
                ...
                this.$store.dispatch('storeBehavioralSkill', { 'title': this.title })
                .then(response => {
                    this.$notify({
                        group: 'success',
                        width : '400px',
                        title: 'success',
                        text: `مهارت ${response.data.data.skill.title} با موفقیت ثبت شد`,
                        duration: 10000,
                        speed: 1000,
                        closeOnClick: true,
            ...
            updateSkill() {
                this.modal_loading = true;
                this.$store.dispatch('updateBehavioralSkill', { 'id': this.id  , 'title' : this.title})
                .then(response => {
                    this.$notify({
                        group: 'success',
                        width : '400px',
            ...
            deleteS ( id , index) {
                ...
                this.modal_loading = true;
                this.$store.dispatch('deleteBehavioralSkill', { 'id': this.id  , 'index' : this.index})
                .then( response => {
                    this.$notify({
                        group: 'success',
                        width : '400px',
        ...
        computed : {
            behavioralSkills(){return this.$store.state.BehavioralSkills.behavioralSkills},
        }
    }
</script>
<style>
