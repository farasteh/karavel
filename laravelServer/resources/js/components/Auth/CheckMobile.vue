                ...
                <vac style="" :left-time="240000"  ref="vac1">
                    <template
                        ...
                        <div class="container-login100-form-btn">
                            <v-bnt :loading="loading" text="تایید" @click.prevent.native="verifyMobile"></v-bnt>
                        </div>
                    </template>
                    <template
                        v-slot:finish>
...
export default {
    ...
    methods:{
        sendSMS : function (){
            this.$store.dispatch('CheckMobile', {'mobile' : this.$store.state.Auth.user.mobile , 'scope' : this.$store.state.Auth.Auth.body.scope})
                .then(success => {
        ...
        verifyMobile : function (){
            ...
            this.loading = true;
            this.$store.dispatch('CheckCodeSMS', {'code_sms' : this.code_sms , 'token' : this.$store.state.Auth.Auth.body.token })
                .then( success => {
        ...
        handler: function (event) {
            ...
            if(this.$store.state.Auth.Auth.body.type != 'register'){
                this.$router.push({name : 'Home'});
                return false;
            }
