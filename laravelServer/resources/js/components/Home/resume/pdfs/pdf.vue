                            ...
                            <div class="col-12 col-md-6 smbox col-narrow sortable-cv-items">
                                <div class="property-item">
                                    <div class="property-item_head" :style="{ borderColor : activeColor}">
                                        <label :style="{ color: activeColor}" class="font-16">اطلاعات تماس </label>
                                    </div>
                                ...
                                <div class="property-item social__list"
                                    ...
                                    <div class="property-item_head" :style="{ borderColor : activeColor}">
                                        <label :style="{ color: activeColor}" class="font-16"> شبکه های اجتماعی </label>
                                    </div>
                                    <div class="property-item_body">
...
export default {
    ...
    created() {
        Axios.get('api/v1/site/pdfResult/' + this.$route.params.id)
            .then(response => {
                this.result = response.data.data
                this.getPdf();
            })
            .catch(({error, status}) => {console.log(error)});
    },
    ...
    methods: {
        ...
        getPdf() {
            let form = {
                'themeNumber': 2,
                'base_id' : this.result.id
            }
            Axios.post('api/v1/member/resume-pdf-options', form)
                .then(res => {
