        ...
        <div class="container-fluid">
            <div class="row">
                        ...
                        <Card title="وضعیت متقاضی" icon="ios-options" :padding="0" shadow >
                            <CheckboxGroup  v-model="filterResume">
                                <Checkbox v-for="(status , index)  in statusList" :key="index" :label="status.id" class="my-2 col-12 col-sm-6 col-md-4">
                                    {{ status.title }}
                                    <span :class="setColor(status.id)">{{ $store.getters.resumeadvertizingStatusCount(status.id) }}</span>
                                </Checkbox>
                            </CheckboxGroup>
                        </Card>
                    </div>
                ...
                <div class="col-12" v-else>
                    ...
                    <div  class="box-A" v-for="(advertizing , index ) in memberAdvertizing" :key="index">
                        ...
                        <div style="display: flex !important;align-items: center !important;justify-content: center;flex-wrap: wrap;" class="row text-center">
                             <div class="col-md-4">
                                   <router-link :to="{name : 'memberAdvertizingSingle' , params : { id: advertizing.id} }">
                                  <h5>
                                      {{ advertizing.title }}
                                  </h5>
                              </router-link>
                            ...
                            <div class="col-md-3">
                                <router-link :to="{name : 'memberAdvertizingSingle' , params : { id: advertizing.id} }" type="button" class="btn btn-link">نمایش وضعیت ارسال رزومه</router-link>
                            </div>
                            <div class="col-md-2">
                                <div :class="setColor(advertizing.status.id)">
                                    {{$store.getters.getStatusTitle(advertizing.status.id)}}
                                </div>
...
export default {
    ...
    computed: {
         memberAdvertizing() {
             return this.$store.getters.resumeStatusAd(this.filterResume)
        },
        statusList() {
            return this.$store.state.resumeStatus.statusList
        },
    },
    watch: {
        statusList() {
